...
- Restaurants implemented end-to-end for the first beta release. Hotels are postponed until after beta; beta work must preserve reusable category boundaries without adding hotel-specific catalogue, product, review, ranking, recommendation, or test features.
...
- Optional public plain-text reviews of restaurants, with no star score or public rating; author declarations, time-limited publication, transparent “not independently verified” disclosure, and a notice-and-action/moderation process must be complete before beta.
...
- Hotel catalogue ingestion, hotel-specific metadata/copy/UI, hotel rankings, hotel recommendations, hotel reviews, and hotel beta research. Hotels are the first planned post-beta category; shared contracts must remain category-compatible in the meantime.

## Confirmed MVP product decisions

1. **Category rollout:** the first beta is restaurant-only. Add hotels as the second category after beta validation. Category-specific behavior must remain behind shared domain, persistence, recommendation, and UI contracts rather than being hard-coded into the restaurant flow, but do not add hotel-specific features during Phases 8–9.
...
8. **Starting geography:** Italy. Import and search Italian restaurants for beta. Defer the Italian hotel import and hotel coverage audit until the post-beta hotel phase. Locality remains an optional filter over each global category list and predicted order.
...
19. **Non-photo place cards:** use `@lucide/svelte` as the sole MVP icon library. Render a reusable, category-themed fallback panel with `UtensilsCrossed` for restaurants, plus the place name, category, and locality. Keep icon selection category-driven so a later hotel phase can add `Hotel` without replacing the component. Do not add a hotel variant before beta, use generic stock photography, or add another icon source unless a validated future category cannot be represented by Lucide.
20. **Beta operations stack:** host the SvelteKit Node application and its database on a netcup VPS, provisionally 4 vCPU, 8 GB RAM, 250 GB NVMe, running Ubuntu or Debian on x86-64. Use Caddy, Docker Compose, and one PostgreSQL server shared by this and other small web apps; isolate every app with its own database, PostgreSQL role, credentials, and permissions limited to that database. Use Cloudflare for domain registration and DNS, Brevo for transactional email, Better Stack for error reporting plus uptime/cron monitoring, and Cloudflare R2 under EU jurisdiction for restricted evidence/object storage and encrypted database backups. Keep the seven most recent nightly `pg_dump` backups and create one weekly backup, retaining each weekly backup for one month. Use GitHub for the source repository and GitHub Actions for CI/CD and heavy scheduled jobs. Add Google OAuth while retaining email/password. Preserve application-owned provider interfaces and least-privilege, environment-separated credentials.
...
The review design is based on [Articles 18–21 of Italian Law 11 March 2026, no. 34](https://www.gazzettaufficiale.it/eli/id/2026/03/23/26G00050/SG), [Regulation (EU) 2022/2065](https://eur-lex.europa.eu/eli/reg/2022/2065/oj/eng) (especially Articles 14, 16 and 17), the Consumer Code rules on online-review transparency, and the [June 2026 AGCM draft guidelines](https://www.agcm.it/dotcmsdoc/consultazioni/Schema_Linee_guida_recensioni_online.pdf). The Law 34/2026 rules apply to the Italian restaurant and tourism sectors. Apply the conservative policy to every Italian restaurant in the beta catalogue rather than making legal behavior depend on incomplete catalogue tags; reassess and approve its accommodation-specific application before hotel reviews are implemented after beta.
...
- Source `amenity=restaurant` for beta from an Italy OSM PBF extract, then refresh it through repeatable imports/diffs. Keep the importer category-extensible, but defer sourcing `tourism=hotel` until the post-beta hotel phase. Do not scrape or use the OSM editing API for bulk catalogue creation.
...
- Run a loose beta coverage audit for Italian restaurants: unusable identities, severe duplicates, missing names/coordinates, stale/closed places, category errors, geographic skews, and any gaps likely to deeply bias the system. Quarantine clearly harmful records and document lesser limitations. Apply the same audit contract to hotels later without adding hotel-specific ingestion or audit work before beta. If a severe systemic issue is found, retain the provider interface and add an ODbL-compatible enrichment source rather than replacing domain contracts.
...
Use the following as the Phase 9 beta target. During Phases 0–8, run the application, PostgreSQL, ingestion, email capture, jobs, artifact storage, analytics, and error capture locally. Keep provider boundaries explicit so infrastructure can change without rewriting domain, authentication, review, or recommendation logic.

- **VPS and application runtime:** deploy the SvelteKit Node application to a netcup VPS, provisionally 4 vCPU, 8 GB RAM, 250 GB NVMe, running Ubuntu or Debian on x86-64. Use Docker Compose for reproducible application and worker services and Caddy for TLS termination and reverse proxying. Validate resource contention, health checks, restart behavior, logs, security updates, deployment rollback, and sustained response times under realistic load, including the other small apps sharing the host.
- **PostgreSQL isolation:** run one PostgreSQL server on the VPS for GustiMei and the other small web apps. Give every app a separate database and PostgreSQL login role with unique credentials and privileges limited to its own database; do not grant cross-database access or routine superuser rights. Separate migration/backup credentials from the runtime role where needed, restrict network exposure, and test connection limits, upgrades, restore, and noisy-neighbor behavior.
- **Domain and DNS:** register the domain and manage authoritative DNS through Cloudflare. Keep registrar and Cloudflare account recovery, DNS changes, DNSSEC where supported, Caddy certificate issuance, and credential rotation in the operational runbook.
- **Source, CI/CD, and heavy jobs:** keep source control on GitHub. Use GitHub Actions for CI/CD and resource-heavy scheduled jobs that should not contend with the shared VPS. Deploy immutable, traceable releases with environment approvals, least-privilege deployment credentials, health verification, and rollback. Never place database dumps, restricted evidence, application secrets, or personal-data exports in workflow artifacts or logs.
- **OSM import/update execution:** own a repeatable TypeScript CLI in this repository. In Phase 9, validate an explicitly triggered GitHub Actions path and enable conservative restaurant updates only after runtime, idempotency, atomic promotion, failure recovery, and cost are proven. Stream Geofabrik input into staging tables, validate counts/checksums/source timestamps, and promote a complete revision atomically. Do not run bulk ingestion in the web process, commit PBF files, or retain full extracts as workflow artifacts.
- **Transactional email:** use Brevo's REST API behind the application-owned email interface for preview/beta/production verification, password reset, security, material terms/privacy changes, data-rights, and strictly necessary service-operation mail. Keep the console/in-memory transport local-only. Authenticate the sending domain with SPF, DKIM, and DMARC; test delivery, bounce/suppression behavior, rate limits, provider branding, expired/reused links, and background delivery before invitations. Send only the recipient and minimum template/action data, keep tokens and action URLs out of analytics and ordinary logs, and review Brevo's DPA, subprocessors, processing locations, and retention configuration before beta. Do not send marketing, promotional, newsletter, or re-engagement email in the MVP.
- **Product analytics:** implement the MVP collector and conversion attribution as first-party code backed by allowlisted domain events in the GustiMei PostgreSQL database. Derive authoritative recommendation exposure-to-visited conversion from domain records. Store no email, name, free-form search text, precise coordinates, full action URLs, or raw comparison pairs in analytics; use internal pseudonymous identifiers and define detailed-event retention and aggregate cleanup before collection. Do not integrate a third-party or cross-site analytics service, browser analytics identifier, pixel, fingerprint, or session replay in the MVP.
- **Error reporting and monitoring:** use Better Stack for error reporting plus uptime and cron/job monitoring, alongside structured redacted service logs on the VPS. Configure application-side scrubbing, conservative telemetry, alert routing, and no session replay. Never send cookies, authorization headers, emails, verification/reset URLs, raw ranking comparisons, restricted evidence, or precise location data. Monitor the public health endpoint and every critical scheduled job without exposing sensitive payloads; retain essential audit and health signals independently if the provider is unavailable.
- **Restricted objects and evidence:** use a private Cloudflare R2 bucket under EU jurisdiction for restricted review evidence and other protected objects. Use separate prefixes or buckets and least-privilege credentials by purpose and environment, server-side encryption, short-lived authorized access, access auditing, lifecycle/deletion rules, and deletion verification. Do not expose the bucket publicly or use it for product analytics.
- **Database backups:** create an encrypted logical `pg_dump` of the GustiMei database every night and upload it directly to a private Cloudflare R2 backup namespace under EU jurisdiction. Retain the seven most recent nightly backups and create one weekly backup, retaining each weekly backup for one month. Keep backup credentials separate from application runtime credentials, alert on dump/upload/retention failures through Better Stack, and run documented restore drills that verify both data recovery and post-restore erasure/evidence-deletion tombstone replay. Back up each co-hosted app independently rather than granting one app access to another app's database.
- **Regional, security, and cost constraints:** keep the VPS, primary database, analytics records, restricted R2 objects, and backups in approved EU locations/jurisdiction. Require TLS, host firewalling, least-privilege and environment-separated secrets, documented patching/retention/deletion, processor/subprocessor review, and an agreed recurring budget before external beta invitations. Pseudonymous rankings remain personal data; do not describe them as anonymous.
...
| Application/Better Stack error records | Target 30 days and the shortest provider setting that remains operationally useful |
...
The ranking UX begins only after login. For beta, the user searches the Italian restaurant catalogue, marks restaurants as visited, and adds them to their one restaurant list. The shared contracts retain category identity so a later hotel phase can add a separate global hotel list without mixing evidence, but no hotel flow is implemented before beta. Locality may narrow catalogue search or a displayed personal list, but adding a place always modifies the global list for that category. The user can start ranking as soon as the list contains two places.
...
- Replace `adapter-auto` with the Node adapter and verify the production build and server locally. Define the hosting/runtime boundary and contract-test seams without deploying to or integrating the Phase 9 netcup VPS stack.
...
**Exit:** clean reproducible baseline, confirmed restaurant-only beta with post-beta hotel compatibility, catalogue/privacy model, and passing CI.
...
- Verification completed: Prettier, ESLint, Svelte diagnostics, repeated Svelte autofixer passes,
  ...
  stopped or modified. This was a one-off historical fallback; the checked-in `db-test` Compose
  service remained the routine local default.
...
- Verification completed: Prettier, ESLint, zero-warning Svelte diagnostics, repeated Svelte autofixer
  ...
  the temporary server was stopped after the suite. This was a one-off historical fallback, not the
  default test setup. The E2E runner now accepts an `E2E_PORT` override because another local process
  occupied its default port 3000.
...
- Verification completed with Prettier, ESLint, zero-warning Svelte diagnostics, repeated Svelte
  ...
  a server with different credentials; the temporary server and data directory were removed. This was
  a one-off historical fallback, not the default `db-test` Compose service used for routine local runs.
...
- Follow-up mailbox testing found that moderation decisions queued a notifier email even for the
  supported anonymous-notice branch, whose notifier email is intentionally empty. The outbox worker
  then treated that empty reference as a user ID and resolved it outside its failure boundary, so the
  oldest poison job returned a 500 from the development mailbox and blocked every later message.
  Anonymous decisions now notify only the author; the notification boundary refuses empty recipients;
  and the worker records invalid or missing recipients as terminal failures, preserves their audit
  rows, skips them on later batches, and continues delivering valid jobs. Database regressions cover
  anonymous decisions, empty recipients, erased/missing users, batch continuation, and non-retry.
...
- What local latency, memory, artifact-size, and quality thresholds must the Phase 7 implementation meet before beta deployment, while leaving room for a later independently validated hotel model?
...
candidate category, and serving gate. Preserve these boundaries for a later hotel implementation,
which must select and validate its model and serving gate independently after beta. Human
comprehension and full-catalogue performance remain Phase 8/9 restaurant-beta gates.
...
- Do the restaurant diagnostics justify the provisional recommendation family and serving gate for beta, or is an ADR/model change required first?
...
- Which restaurant-specific assumptions still leak across shared category contracts and must be removed before beta without implementing hotel behavior now?
...
- Proxy explicit-submit location searches through an application-owned geocoder boundary. The local
  ...
  attribution, and no analytics capture of search text or precise coordinates.
- Record recommendation exposures only for previously unvisited markers that enter the rendered
...
best-effort public OSM tile/Nominatim endpoints before material traffic if measured usage, service
terms, reliability, or capacity requires a dedicated OSM-derived provider; this does not block local
work through Phase 8.

### Phase 8 — Restaurant beta architecture and local readiness

- Keep the beta product restaurant-only. Do not import hotels or add hotel-specific metadata, copy, screens, reviews, ranking behavior, recommendation artifacts, research measures, or tests in this phase.
- Audit category boundaries across catalogue providers, place identity, ranking lists and revisions, comments, public reviews and cases, recommendation evidence/artifacts, routes, localization, analytics, exports, and deletion. Replace leaked restaurant assumptions with reusable category contracts only where needed for the restaurant implementation; do not speculate new hotel requirements or add dormant hotel branches.
- Require every category-owned record, cache key, job lock, artifact namespace, authorization query, export, deletion path, and analytics aggregation to carry or derive an explicit category where cross-category mixing would otherwise become possible. Preserve the invariant that future hotel evidence and artifacts cannot mix with restaurant data.
- Keep provider integrations behind the existing email, object/evidence storage, error-reporting, health/cron, job, and artifact-store boundaries. Add contract tests and production configuration validation needed for the Phase 9 providers without provisioning them in Phase 8.
- Prepare a production Docker image and Compose topology for the SvelteKit Node app and workers, Caddy routing, and the single PostgreSQL server boundary. Define separate GustiMei runtime, migration, backup, deployment, and operator credentials with only the permissions each requires; document how co-hosted apps remain database- and credential-isolated.
- Define GitHub Actions workflows for CI, immutable build/deploy/rollback, manual restaurant OSM import, model rebuild/promotion, and other heavy scheduled jobs. Keep secrets, dumps, restricted evidence, and personal-data exports out of artifacts and logs; make every mutation idempotent, locked, observable, and safely retryable.
- Define the private Cloudflare R2 object layout, EU-jurisdiction requirement, encryption, purpose/environment separation, lifecycle rules, evidence access audit, deletion verification, and direct encrypted backup upload contract. Encode the retention policy as the seven most recent nightly dumps plus one dump per week, with each weekly dump retained for one month.
- Define Better Stack error-reporting, uptime-check, and cron-monitor contracts, including redaction, minimum-payload check-ins, alert ownership, escalation, and behavior when monitoring is unavailable.
...
- What is the approved recurring beta budget, including the netcup VPS, Cloudflare domain/DNS and R2 usage, Brevo, Better Stack, taxes, and operational headroom?
- Is the selected netcup VPS plan confirmed at approximately 4 vCPU, 8 GB RAM, and 250 GB NVMe, and will it run Ubuntu or Debian? Which region, OS version, filesystem, patching window, and resource reservations for co-hosted apps are approved?
- What numeric latency, availability, email-delivery, database-size/connection, backup/restore, object-deletion, scheduled-job, and model-rebuild thresholds gate invitations?
- Which restaurant beta locality, cohort size/composition, recruitment method, research scripts, incentives, and success interpretation are approved in the external research brief?
- Has legal review approved mandatory reciprocal contribution, the 18+ approach, notices/Terms, provider processing, R2 EU-jurisdiction configuration, retention, and the Phase 9 research/data procedures; if not, which documented fallback must be activated before invitations?
- Has counsel approved the final review-policy/Terms copy, declaration and authenticity measures, date arithmetic/display, DSA classification and redress obligations, owner/delegate procedure, moderation grounds/SLAs, transparency duties, evidence/audit retention, and any changes from the final AGCM guidelines? If not, external beta invitations remain blocked.
- Who owns netcup, Cloudflare, GitHub, Brevo, Better Stack, and Google OAuth administration, billing, credential recovery, incident response, moderation escalation, deployment approval, backup restore, and rollback decisions?

### Phase 9 — Infrastructure hardening and restaurant beta release

- Threat-model the shared VPS and Docker/host boundary, Caddy and TLS configuration, PostgreSQL roles and network exposure, GitHub Actions deployment and scheduled-job credentials, R2 evidence/backup access, Better Stack telemetry, Brevo delivery, Google OAuth, authentication, personal-comment and public-review/case authorization/IDOR, stored-content rendering, evidence upload/download and malware, catalogue ingestion, comparison writes, rate limiting, CSRF, XSS, abuse, and expiry/retention bypass paths.
- Provision the selected netcup VPS, install and harden the approved Ubuntu or Debian x86-64 release, configure host firewalling and security updates, and deploy Caddy plus the Docker Compose application/worker topology. Configure environment-separated secrets, resource limits, health checks, restart policies, structured redacted logs, and an operator-only initial environment using synthetic or authorized internal-test data. Prove deployment and rollback before external invitations.
- Provision one PostgreSQL server for the VPS and create a dedicated GustiMei database and login role with unique credentials and permissions only on that database. Keep other apps in separate databases with separate roles; verify that GustiMei cannot connect to or read them. Use distinct least-privilege migration, backup, and operational access where required; test connection saturation, migration rollout/rollback, PostgreSQL upgrades, disk pressure, and noisy-neighbor isolation.
- Register the domain and manage DNS through Cloudflare. Configure DNS records, account recovery, DNSSEC where supported, Caddy certificate issuance/renewal, origin exposure controls, and a tested DNS/registrar incident procedure.
- Implement and exercise the Brevo transactional-email adapter. Verify SPF, DKIM, DMARC, bounce/suppression behavior, rate limits, provider branding, expired/reused links, durable background delivery, redacted logs, and provider-contract tests before invitations.
- Integrate Better Stack for scrubbed error reporting, release association, alerts, public uptime checks, and cron/job monitoring. Exercise alert routing, missed-check-in and job-failure detection, quota/provider-failure behavior, and the rule that monitoring payloads contain no secrets, personal content, raw comparisons, precise locations, or restricted evidence. Retain provider-independent health and audit records.
- Provision private Cloudflare R2 storage under EU jurisdiction for restricted review evidence and protected objects. Enforce purpose/environment isolation, least-privilege credentials, encryption, short-lived access, file validation/malware controls, access auditing, lifecycle rules, deletion deadlines, and deletion verification under retries and provider failure.
- Implement nightly encrypted `pg_dump` backups of the GustiMei database directly to a separate private R2 backup namespace. Automate retention of the seven most recent nightly dumps plus one dump per week, retaining each weekly dump for one month; report every backup and retention job to Better Stack, and prove a clean restore plus migration compatibility and erasure/evidence-deletion tombstone replay. Never place dumps in GitHub Actions artifacts or grant the application runtime backup credentials.
- Use GitHub as the source repository and GitHub Actions for CI/CD and heavy scheduled jobs, including the runner-neutral restaurant OSM importer and recommendation model rebuilds. Protect production environments, pin actions, minimize permissions, use short-lived or narrowly scoped deployment/job credentials, prevent untrusted code from reaching secrets, and keep sensitive inputs/outputs out of logs and artifacts. Enable schedules only after manual runs prove runtime, locking, idempotency, atomic promotion, failure recovery, monitoring, and cost.
- Add Sign in with Google through Better Auth using separate development/preview/production OAuth clients, exact redirect origins, and minimum authentication scopes. Link social identities only through Better Auth's verified, explicit account-linking rules; test duplicate email, provider-email changes, revoked consent, cancelled callbacks, state/PKCE and redirect validation, sessions, deletion, and recovery. Retain email/password and disable local email surrogates outside local/test environments.
- Complete accessibility testing for keyboard, screen reader, contrast, touch targets, zoom, reduced motion, and both locales.
- Complete and obtain legal review of the DPIA, processing record, reciprocal-contribution lawful-basis assessment and fallback, final AGCM-guideline compliance matrix, DSA/Consumer Code duties, processor/transfer register, 18+ approach, Privacy Notice, Terms/review rules/moderation policy, cookie/storage notice, provider attribution, retention schedule, and the final netcup/Cloudflare/GitHub/Brevo/Better Stack/Google configurations. Do not invite users with placeholder legal text or a mismatch between policy, disclosure, infrastructure, and stored/model inputs.
- Implement and verify self-service JSON/CSV access/export, comment deletion, review withdrawal/deletion, ranking-category deletion, account deletion, processing restriction, recommendation-evidence exclusion and rebuild, retention jobs, privacy-request tracking, and restored-backup tombstone replay.
- Verify that the beta ships no marketing email, cross-site/third-party analytics, non-essential tracking, fingerprinting, pixels, advertising identifiers, or session replay, and that transactional templates contain no promotional material.
- Run responsive and cross-browser end-to-end tests of the complete restaurant flow: sign-up and Google login, visited-place selection, ranking and insertion, recommendation, reviews and moderation/redress, locale switching, exports/deletion, job failure recovery, and infrastructure degradation.
- Exercise the staffed beta moderation/incident runbook, urgent escalation, moderator conflict/absence backup, queue/decision/deletion alerts, evidence-access audit, VPS compromise/recovery, database restore, credential rotation, failed deployment rollback, DNS outage/change, Brevo failure, Better Stack failure, and R2 access/deletion failure before invitations. Use only synthetic non-sensitive evidence fixtures for drills.
- Verify beta synthetic-data labelling and isolation, then run the approved restaurant-only private-beta cohort and research method. Collect the first external real-user evidence under the documented lawful bases, notices, and participant controls; use separate research consent where a distinct research activity requires it.
- After sufficient beta evidence exists, repeat the leakage-safe restaurant recommendation evaluation, report synthetic, internal-testing, and beta results separately, recalibrate the support/personalization gate, compare relevance with the global prior, and review delayed predicted-versus-actual agreement before general-release or hotel-expansion claims.

**Exit:** an operable restaurant-only private beta has run on the netcup VPS stack with Caddy, Docker Compose, isolated PostgreSQL access, Cloudflare domain/DNS and EU R2 storage/backups, Brevo transactional email, Better Stack reporting/monitoring, GitHub Actions CI/CD/jobs, and Google OAuth. Rollback, support, moderation/redress, evidence deletion, privacy, backup/restore, and measurement procedures are proven well enough to decide whether to proceed to general release and the separately scoped hotel phase.

**Open questions to answer before general release or hotel implementation:**

- Did the restaurant beta meet the approved ranking-completion, recommendation-relevance, catalogue coverage, usability, privacy, moderation, and reliability targets?
- Did external-cohort evaluation confirm or change the restaurant model family, candidate universe, pagination behavior, personalization/support gate, and cold-start presentation?
- Are measured VPS capacity and co-tenancy isolation, PostgreSQL operations, recurring costs, backup/restore performance, email delivery, monitoring, scheduled-job reliability, incident handling, and provider/data-processing terms acceptable for continued operation?
- Did review publication, notice handling, reasoned decisions/redress, expiry, evidence deletion, abuse controls, accessibility, and moderator coverage meet the approved legal/product targets without leaking content into recommendation systems?
...
- What product evidence and separately approved scope justify starting hotel ingestion and hotel-specific catalogue, review, ranking, recommendation, localization, compliance, testing, and research work?
